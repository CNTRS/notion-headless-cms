...
- [x] 2.4 Verify `pages.retrieve()` and `blocks.children.list()` still compile with v5 types (no changes expected)
...
- [x] 3.3 Verify all existing handlers for pages and blocks still match v5 URL patterns
...
- [x] 5.4 Copy the 11 v5 fixtures from `openspec/changes/upgrade-notion-sdk/fixtures/` into `src/test/msw/fixtures/` and delete the obsolete `databases.query.*.json`
